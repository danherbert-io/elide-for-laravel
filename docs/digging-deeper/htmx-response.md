...
If you wish this behaviour to apply to all responses generated by `Htmx::render()` you can instruct the service to do so:

```php
Htmx::omitRenderedChildPartials();
// or
Htmx::omitRenderedChildPartials(shouldOmit: true);
```

You can also disable this in the same manner:

```php
Htmx::omitRenderedChildPartials(shouldOmit: false);
```
